...
@section('main')
    ...
    <h2>{{ $is_update ? '编辑' : '添加' }}角色</h2>
    <div class="container">
        <form method="POST" action="{{ $is_update ? action('Permission\RoleController@update', ['id' => $item->id]) : action('Permission\RoleController@store') }}">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    @include('permission.role.layouts.permissions')
                </div>
            </div>
            <div class="form-group row">
